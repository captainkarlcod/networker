			...
			class CSCO_Kirki_Installer extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'Please install the Kirki plugin to take full advantage of this theme\s customizer capabilities', 'networker' ); ?>
					</p>
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $this->get_plugin_install_url() ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'networker' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Install Now', 'networker' ); ?>
					</a>
					<?php
				}
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'networker' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'networker' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'networker' ); ?>
					</a>
					<?php
				}
				/**
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'networker' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'networker' ); ?>">
						<?php esc_html_e( 'Don\'t show this again', 'networker' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_html_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'networker' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
							jQuery.post( ajaxurl, {
